...
- 관리자 화면의 대량 업로드는 선택 파일을 10개 단위 제출물로 순차 전송해 브라우저와 네트워크 부담을 줄인다.
- 대량 선택 시 브라우저에서 SHA-256을 계산해 완전히 같은 사진을 먼저 제외한다. 크롭·보정·압축된 유사 사진 AI 검사는 별도 서비스와 개인정보 전송 동의 결정 후 구현한다.
- `admin`, `family`, `reviewer` Cognito 그룹을 인프라 템플릿에 정의했다. 가족 계정 생성과 실제 그룹 적용에는 사용할 이메일과 AWS 스택 업데이트가 필요하다.
- 승인 대기 목록은 여러 묶음 선택, 전체 선택과 순차 일괄 승인을 지원한다.
