...
export default function MobileNav({ language = 'ko' }: { language?: 'ko' | 'en' }) {
  ...
  return <>
    <nav className="mobile-bottom-nav" aria-label={ko ? '모바일 바로가기' : 'Mobile shortcuts'}>
      ...
      <Link href={ko ? `${memories}#notices` : '/en/photos'}><span aria-hidden="true">●</span><strong>{ko ? '모임·소식' : 'Photos'}</strong></Link>
    ...
    {open && <div className="mobile-menu-overlay" role="dialog" aria-modal="true" aria-label={ko ? '전체 메뉴' : 'Full menu'}>
      ...
      <nav>
        <Link href={ko ? '/#life' : '/en#life'} onClick={close}>{ko ? '교수님의 삶' : 'His Life'}</Link>
        <Link href={ko ? '/#teaching' : '/en#teaching'} onClick={close}>{ko ? '가르침과 연구' : 'Teaching & Research'}</Link>
        <Link href={ko ? '/visiting' : '/en/visiting'} onClick={close}>{ko ? '아버님 찾아가는 길' : 'Visiting His Resting Place'}</Link>
        <Link href={ko ? '/photos' : '/en/photos'}>{ko ? '사진첩' : 'Photo Archive'}</Link>
