...
export default function HomeMemories({ language = 'ko' }: { language?: 'ko' | 'en' }) {
  ...
  const memoryCard = (memory: PublicMemory, featured = false) => {
    ...
    <div className="shared-story-layout">
      {featured && memory.photos[0] && <Link className="shared-media-link" href={storyHref} aria-label={memory.title}>
      <div className="shared-story-media">
        <img className="shared-story-photo" src={memory.photos[0].url} alt="" loading="lazy" />
      </div>
      </Link>}
      <div className="shared-story-copy"><p className="shared-connection">{connection(memory.group)}</p><h3>{memory.title}</h3>
        ...
        <div className="shared-story-end"><Link href={storyHref}>{en ? 'Read the Memory' : '이야기 읽기'} <span aria-hidden="true">→</span></Link><MemoryHeart id={memory.id} initialCount={memory.likeCount} language={language} /></div>
      </div>
    </div>
  </article>;
  };
