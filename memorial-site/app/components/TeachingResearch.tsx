...
// The original ten entries retain the family CV transcription. Additional entries
// require a source-level check of Jung's correspondence and research continuity.
...
export default function TeachingResearch({ language = 'ko' }: { language?: 'ko' | 'en' }) {
  ...
  return <section className="legacy section-shell research-section" id="teaching" lang={language}>
    ...
    <div className="research-story"><h3>{en ? 'A Research Journey' : '연구가 이어온 길'}</h3><p className="research-context">{en ? 'Selected publications trace these overlapping strands of his research.' : '대표 논문으로 살펴본 연구의 흐름입니다. 각 분야는 시기를 겹치며 함께 이어졌습니다.'}</p><ol className="research-timeline">{(en ? englishChapters : chapters).map(chapter => <li key={chapter.years}><span className="research-years">{chapter.years}</span>{chapter.field && <p className="research-field">{chapter.field}</p>}<h4>{chapter.title}</h4><p>{chapter.body}</p></li>)}</ol>
      <div className="research-publications"><p className="section-kicker">Selected Publications</p><h3>{en ? 'Selected Publications' : '대표 논문'}</h3><p className="research-context">{en ? 'Three papers from the family-held CV introduce these strands of his work.' : '가족이 보관한 이력서에서, 세 연구 흐름을 보여주는 논문을 먼저 소개합니다.'}</p><ul>{featured.map(index => <Publication key={index} index={index} en={en} />)}</ul><details><summary><span className="research-expand">{en ? `View all ${publications.length} selected publications` : `대표 논문 전체 보기 · ${publications.length}편`}</span><span className="research-collapse">{en ? 'Show fewer publications' : '추가 논문 접기'}</span></summary><p className="research-context">{en ? 'Further papers from his CV, followed by two later studies continuing his synthesis research. The selection is not a complete bibliography.' : '이력서의 나머지 논문과, 기존 합성 연구를 이어간 최근 논문 2편입니다. 전체 논문 목록이 아닌 선별 기록입니다.'}</p><ul>{publications.map((_, index) => featured.includes(index) ? null : <Publication key={index} index={index} en={en} />)}</ul></details><p className="research-source">{en ? 'Sources: family-held CV and original publication records. Collaborative work retains its author credits.' : '자료: 가족 보관 이력서 및 논문 원문 기록. 공동 연구의 저자 표기를 존중합니다.'}</p></div>
