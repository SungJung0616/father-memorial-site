...
function MemoryDetailContent() {
  ...
  return <main className="memory-detail-page">
    ...
    <article className="memory-detail-card">
      <div className="memory-detail-heading"><p>{memory.category || memory.group}</p><h1>{memory.title}</h1><div><span>{memory.group}</span><time>{new Date(memory.submittedAt).toLocaleDateString('ko-KR')}</time></div></div>
