...
export default function PhotosPage(){
  ...
  return <main className="photos-page"><header><Link href="/en">← In Memory of Professor Jung</Link><div><p className="section-kicker">Life in Photos</p><h1>Photo Archive</h1><p>가족이 공개 승인한 추억 속 사진을 한곳에서 봅니다. 같은 사진 자료가 추억 이야기와 Photo Archive에 함께 연결됩니다.</p></div><Link href="/en/contribute">Share Photos</Link></header>
    ...
    <section className="published-photo-grid" aria-label="Published photographs">{visible.map(photo=><Link href={`/community/story?id=${encodeURIComponent(photo.memoryId)}&lang=en`} key={photo.id}><img src={photo.url} alt={photo.title}/><div><span>{({ '가족': 'Family', '가족·친지': 'Family', '친구': 'Friends', '제자': 'Students', '교수·학계': 'Colleagues', '행사': 'Events' } as Record<string,string>)[photo.category] || 'Memories'}</span><h2>{photo.title}</h2><p>{photo.body||'A cherished memory shared with this photograph.'}</p></div></Link>)}</section>
  </main>;
}
