...
export default function VisitingPage() {
  return <main className="visit-page">
    <header className="visit-header"><Link href="/">← 故 정영훈님</Link><Link href="/community">추억 이야기</Link><Link href="/en/visiting" lang="en">English</Link></header>
...
<div className="visit-steps">{steps.map((step, index) => { const src = `/visiting/${step.image}.${step.id === 'lake' ? 'png' : 'jpg'}`; return <section className="visit-step" id={step.id} key={step.id}><div className="visit-copy"><span className="visit-number">0{index + 1}</span><h2>{step.title}</h2><p>{step.text}</p>{step.id === 'inside' && <figure className="visit-route"><a href="/visiting/baekhap-route.png" target="_blank" rel="noopener noreferrer" aria-label="백합 방향 경로 지도 크게 보기"><img src="/visiting/baekhap-route.png" width="278" height="408" loading="lazy" alt="호수 옆 도로에서 백합 방향으로 이동하는 길을 노란색으로 표시한 가족 제공 지도" /></a><figcaption>가족이 표시한 이동 경로 · 누르면 크게 볼 수 있습니다.</figcaption></figure>}<a href={src} target="_blank" rel="noopener noreferrer" aria-label={`${step.title} 사진 크게 보기`}>사진 크게 보기 ↗</a></div><a className={`visit-photo${step.id === 'lake' ? ' visit-map' : ''}`} href={src} target="_blank" rel="noopener noreferrer"><img src={src} alt={step.alt} loading="lazy" width={step.id === 'lake' ? 274 : 1368} height={step.id === 'lake' ? 223 : 1824} /></a></section>; })}</div>
